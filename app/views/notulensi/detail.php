...
<style>
  ...
  .foto-gallery-item:hover { border-color: var(--primary-light); transform: scale(1.02); }
  .foto-gallery-item img   { width: 100%; height: 100%; object-fit: cover; display: block; }
  .foto-gallery-item .foto-overlay {
    position: absolute; inset: 0; background: rgba(0,0,0,0);
    display: flex; align-items: center; justify-content: center;
    transition: background .2s; color: white; font-size: 22px; opacity: 0;
  }
  .foto-gallery-item:hover .foto-overlay { background: rgba(0,0,0,.3); opacity: 1; }
...
<div class="card">
  <div class="card-header">
    ...
    <div style="display:flex;gap:8px">
      <a href="<?= $baseUrl ?>/index.php?url=notulensi/edit/<?= $notulensi['id'] ?>" class="btn btn-warning btn-sm">
  ...
  <div class="card-body">
    ...
    <div class="form-group">
      ...
      <div class="foto-gallery">
        ...
        <div class="foto-gallery-item" onclick="openLightbox(<?= $fi ?>)" title="Klik untuk perbesar">
          <img src="<?= $baseUrl ?>/public/uploads/dokumentasi/<?= htmlspecialchars($foto['filename']) ?>"
    ...
    <!-- Dokumen Pendukung — menggunakan partial -->
...
<script>
var lbFotos = <?= json_encode(array_map(
    fn($f) => BASE_URL . '/public/uploads/dokumentasi/' . $f['filename'],
...
function renderLightbox() {
  ...
  var show = lbFotos.length > 1;
  document.querySelector('.lightbox-nav.prev').style.display = show ? '' : 'none';
  document.querySelector('.lightbox-nav.next').style.display = show ? '' : 'none';
}
